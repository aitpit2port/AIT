...
async function doLogin(){
  ...
  try{
    const auth=await apiRequest('checkAuth',{email,password:pass});
    ...
    window.USER_PASS=pass; setSavedEmail(email); applyAuth(auth,email); saveAuthSession(auth,email);
...
async function verifyUserEmail(email){
  ...
  if(window.USER_PASS){
    const auth=await apiRequest('checkAuth',{email:email,password:window.USER_PASS||''});
...
const PAGES=['exec','timeline','budget','expenses','cost','cashflow','risks','vendors','performance','production','quality','mines'], ICONS={exec:'OV',timeline:'TL',budget:'BD',expenses:'EX',cost:'CA',cashflow:'CF',risks:'RK',vendors:'VD',performance:'PR',production:'PD',quality:'QL',mines:'MP',admin:'US'};
