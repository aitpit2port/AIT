...
function renderWorkflowTasks(){
  ...
  if(!workflowPerm('manage_tasks')){
    const departments=unique(allTasks.map(task=>task.department).filter(Boolean));
    pane.innerHTML='<div class="workflow-topline workflow-task-heading"><div><h3>متابعة تنفيذ المهام</h3><p>هذه الصفحة للمشاهدة فقط. تعديل وإغلاق البنود يتم من تبويب البلان اليومي.</p></div></div><div class="workflow-task-filter"><div class="field"><label>القسم</label><select class="select" id="workflowTaskDepartment"><option value="">كل الأقسام</option>'+departments.map(dep=>'<option value="'+esc(dep)+'">'+esc(dep)+'</option>').join('')+'</select></div><span class="workflow-task-count" id="workflowTaskCount"></span></div><div class="workflow-task-list" id="workflowTaskList"></div>';
    const draw=()=>{const department=$('workflowTaskDepartment').value,tasks=allTasks.filter(task=>!department||String(task.department)===department);$('workflowTaskCount').textContent=tasks.length+' مهمة مفتوحة';$('workflowTaskList').innerHTML=tasks.length?tasks.map(task=>'<article class="workflow-task-card"><div class="workflow-task-meta"><div><b>'+esc(task.employee_name||'—')+'</b><span>'+esc(task.department||'—')+' · '+esc(task.plan_date||'—')+'</span></div><span class="workflow-task-state">عرض فقط</span></div><div class="workflow-task-description"><label>المهمة المطلوبة</label><p>'+esc(task.task_description||'—')+'</p></div><div class="workflow-task-description"><label>ما تم عمله</label><p>'+esc(task.progress_notes||'—')+'</p></div></article>').join(''):'<div class="empty workflow-task-empty">لا توجد مهام مفتوحة.</div>'};
    $('workflowTaskDepartment').onchange=draw;draw();return;
  }
  const departments=unique(allTasks.map(task=>task.department).filter(Boolean));
  pane.innerHTML=
