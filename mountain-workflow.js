...
function open(initialPane='people',afterLoad){const modal=$('inputModal');modal.classList.add('workflow-modal','show');$('modalTitle').textContent='الإدخال والتشغيل اليومي';$('modalActions').innerHTML='<div id="workflowSaveFeedback" class="save-feedback"></div><button class="btn ghost" id="workflowClose">إغلاق</button>';$('modalBody').innerHTML='<div class="workflow-shell"><div class="workflow-tabs"><button class="workflow-tab active" data-wtab="people">الأفراد والمهام</button><button class="workflow-tab" data-wtab="vehicles">السيارات المتاحة</button><button class="workflow-tab" data-wtab="leaves">الإجازات</button></div><section class="workflow-pane active" id="w-pane-people"></section><section class="workflow-pane" id="w-pane-vehicles"></section><section class="workflow-pane" id="w-pane-leaves"></section></div>';$('workflowClose').onclick=()=>{modal.classList.remove('show','workflow-modal')};document.querySelectorAll('[data-wtab]').forEach(b=>b.onclick=()=>switchPane(b.dataset.wtab));return loadWorkflow().then(()=>{renderPeople();renderVehicles();renderLeaves();switchPane(initialPane);if(typeof afterLoad==='function')afterLoad()}).catch(e=>feedback(e.message,false));}
...
async function loadWorkflow(){try{WDATA=await MountainCore.api('workflowdashboard',{date:MountainCore.today()});WDATA.route_points=WDATA.route_points||[]}catch(e){const d=data();WDATA={employees:(d.filters&&d.filters.employees)||[],vehicles:d.vehicles||[],tasks:[],plans:[],vehicle_plans:[],leaves:d.leaves||[],departments:(d.filters&&d.filters.departments)||[],route_points:splitList(d.settings&&d.settings.vehicle_route_point_options)}}}
...
async function savePeoplePlan(){
  try{
    ...
    await loadWorkflow();renderPeople();MountainCore.loadData(true);
...
function registeredVehicles(){
  return (WDATA.vehicles||[]).filter(v=>{
    ...
    return active&&hasRegisteredNumber&&String(v.vehicle_status||'available').toLowerCase()!=='inactive';
  });
}
...
function openEmployeeModal(){
  ...
  $('saveNewEmployee').onclick=async()=>{
    const feedbackEl=$('employeeModalFeedback');
    try{
      ...
      await call('saveemployee',{
        employee_name:name,phone:phone,job_title:$('newEmpJob').value,department:deps.join(' | '),mine_id:'المنجم الرئيسي',
      ...
      feedbackEl.textContent='تم حفظ الموظف بنجاح في Google Sheets.';feedbackEl.className='save-feedback success';
      await MountainCore.loadData(true);await loadWorkflow();renderPeople();
...
async function deleteVehicleRecord(id,button){
  ...
  try{
    ...
    await loadWorkflow();renderVehicles();
...
function openVehicleModal(id){
  ...
  $('saveWorkflowVehicle').onclick=async()=>{
    ...
    try{
      ...
      await call('savevehicle',{vehicle_id:id||'',vehicle_type:type,vehicle_brand:type,plate_number:plate,vehicle_name:name||type+' '+plate,allowed_driver_ids:selectedDrivers.join('|'),allowed_driver_names:selectedNames.join('|'),route_points:route.join('|'),vehicle_status:'available',active:true,capacity_passengers:Number($('wvCapacity').value||4)});
      feedbackEl.textContent='تم حفظ السيارة وخط السير بنجاح.';feedbackEl.className='save-feedback success';
      await loadWorkflow();renderVehicles();setTimeout(()=>layer.remove(),650)
    }catch(e){feedbackEl.textContent=e.message;feedbackEl.className='save-feedback error'}
  };
}
...
async function loadTasks(){try{const r=await MountainCore.api('workflowdashboard',{date:MountainCore.today()});WDATA=r;renderTaskPage()}catch(e){if($('openTasksBody'))$('openTasksBody').innerHTML='<tr><td colspan="8" class="empty">'+esc(e.message)+'</td></tr>'}}
...
function bind(){const btn=$('inputBtn');if(btn)btn.onclick=()=>open('people');document.querySelectorAll('[data-quick-op]').forEach(b=>b.onclick=()=>openQuick(b.dataset.quickOp));document.querySelectorAll('.tab-btn').forEach(b=>{if(b.dataset.tab==='tasks')b.addEventListener('click',loadTasks)});if($('taskDepartmentFilter'))loadTasks()}
