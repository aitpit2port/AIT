...
async function savePeoplePlan(){
  ...
  try{
    ...
    Promise.allSettled([MountainCore.loadData(true),loadWorkflow(true)]).then(()=>{if($('w-pane-people'))renderPeople();if($('w-pane-leaves'))renderLeaves()});
...
function renderLeaves(){
  ...
  const today=MountainCore.today();
  const typeOptions='<option value="rotation_leave">إجازة دورية</option><option value="annual">إجازة سنوية</option><option value="mission">مأمورية</option><option value="rest">راحة</option><option value="absent">غياب</option><option value="sick_leave">إجازة مرضية</option>';
...
async function confirmReturn(btn){
  try{
    ...
    feedback('تم تأكيد عودة الموظف وظهوره في صفحة المهام.',true);renderLeaves();renderPeople();
