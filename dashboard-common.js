(function(){
  ...
  window.AIT_CONFIG = Object.freeze({
    ...
    apiVersion: '2026-07-mobile-session-permissions-v1'
  });


  // One strict permission resolver is shared by login, shell and standalone pages.
  // Aliases only normalize names for the SAME page; they never grant access to a
  // different dashboard page (for example Mines Analytics and Gold Map stay separate).
  var PAGE_PERMISSION_ALIASES = Object.freeze({
    exec:['exec','executive','executive_summary'],
    performance:['performance'],
    production:['production'],
    samples:['samples','quality'],
    equipment:['equipment'],
    mountain_entry:['mountain_entry','mountainentry','mountain_entry_workforce'],
    mines:['mines','mine_analytics'],
    goldmap:['goldmap','gold_map','egypt_mines_map'],
    timeline:['timeline'],
    budget:['budget'],
    expenses:['expenses'],
    cost:['cost'],
    ...
    vendors:['vendors'],
    spareparts:['spareparts','spare_parts'],
    approvals:['approvals'],
    risks:['risks']
  });
  var PAGE_FILE_KEYS = Object.freeze({
    'exec.html':'exec','performance.html':'performance','production.html':'production',
    'samples.html':'samples','equipment.html':'equipment','mountain-entry.html':'mountain_entry',
    'mines.html':'mines','egypt_mines_map.html':'goldmap','timeline.html':'timeline',
    'budget.html':'budget','expenses.html':'expenses','cost.html':'cost','cashflow.html':'cashflow',
    'vendors.html':'vendors','spareparts.html':'spareparts','approvals.html':'approvals','risks.html':'risks'
  });
  function normalizePermissionName(value) {
    return String(value == null ? '' : value).trim().toLowerCase().replace(/[\s\-]+/g,'_');
  }
  function truthyPagePermission(value) {
    if (value === true || value === 1) return true;
    var text = normalizePermissionName(value);
    return ['true','1','yes','y','ok','checked','active','enabled','مفعل','نعم','صح','✓'].indexOf(text) >= 0;
  }
  function pagePermissionNames(pageKey) {
    var key = normalizePermissionName(pageKey);
    var aliases = PAGE_PERMISSION_ALIASES[key] || [key];
    var names = [];
    aliases.forEach(function(alias){
      alias = normalizePermissionName(alias);
      if (!alias) return;
      if (names.indexOf(alias) < 0) names.push(alias);
      if (names.indexOf('page_' + alias) < 0) names.push('page_' + alias);
    });
    return names;
  }
  function userHasPage(user, pageKey) {
    var u = user || {};
    var rawPages = u.pages && typeof u.pages === 'object' ? u.pages : {};
    var pages = {};
    Object.keys(rawPages).forEach(function(key){ pages[normalizePermissionName(key)] = rawPages[key]; });
    var flat = {};
    Object.keys(u).forEach(function(key){ flat[normalizePermissionName(key)] = u[key]; });
    var allowedRaw = Array.isArray(u.allowed_pages) ? u.allowed_pages : String(u.allowed_pages || '').split(/[|,;\s]+/);
    var allowed = allowedRaw.map(normalizePermissionName).filter(Boolean);
    var names = pagePermissionNames(pageKey);
    for (var i = 0; i < names.length; i++) {
      var name = names[i];
      if (truthyPagePermission(pages[name]) || truthyPagePermission(flat[name]) || allowed.indexOf(name) >= 0) return true;
    }
    return false;
  }
  function currentPageKey() {
    var file = String(location.pathname || '').split('/').pop().toLowerCase();
    return PAGE_FILE_KEYS[file] || '';
  }
  window.AITPageAccess = Object.freeze({
    aliases: PAGE_PERMISSION_ALIASES,
    files: PAGE_FILE_KEYS,
    truthy: truthyPagePermission,
    normalize: normalizePermissionName,
    names: pagePermissionNames,
    isAllowed: userHasPage,
    currentKey: currentPageKey
  });
  var CURRENT_PAGE_KEY = currentPageKey();
  if (CURRENT_PAGE_KEY) {
    document.documentElement.dataset.pageKey = CURRENT_PAGE_KEY;
    document.documentElement.classList.add('ait-access-pending');
  }

  // Mobile browsers and embedded webviews can block or evict one storage
  // mechanism. Keep the session in both browser stores and keep only the small
  // token in a first-party cookie (full user JSON can exceed the 4 KB cookie limit).
  var SESSION_KEY = 'ait_auth_session';
  var TOKEN_COOKIE_KEY = 'ait_auth_token';
  ...
  function storageGet(store, key) { try { return store && store.getItem ? (store.getItem(key) || '') : ''; } catch (e) { return ''; } }
  function storageSet(store, key, value) { try { if (store && store.setItem) { store.setItem(key, value); return true; } } catch (e) {} return false; }
  function storageRemove(store, key) { try { if (store && store.removeItem) store.removeItem(key); } catch (e) {} }
  ...
  function cookieSet(key, value) {
    try { document.cookie = key + '=' + encodeURIComponent(value) + '; path=/; max-age=604800; SameSite=Lax; Secure'; return true; } catch (e) { return false; }
  }
  ...
  function storageDiagnostic() {
    var result = { local: false, session: false, cookie: false, webview: false };
    var testKey = '__ait_storage_test__';
    result.local = storageSet(window.localStorage, testKey, '1');
    result.session = storageSet(window.sessionStorage, testKey, '1');
    storageRemove(window.localStorage, testKey);
    storageRemove(window.sessionStorage, testKey);
    result.cookie = cookieSet(testKey, '1');
    cookieRemove(testKey);
    result.webview = /FBAN|FBAV|Instagram|Line\/|wv\)|; wv|WhatsApp/i.test(navigator.userAgent || '');
    return result;
  }
  window.AITSession = Object.freeze({
    get: function () {
      var bridgeRaw = readNavigationBridge();
      var localRaw = storageGet(window.localStorage, SESSION_KEY);
      var sessionRaw = storageGet(window.sessionStorage, SESSION_KEY);
      var legacyCookieRaw = cookieGet(SESSION_KEY);
      var value = parseSession(bridgeRaw) || parseSession(localRaw) || parseSession(sessionRaw) || parseSession(legacyCookieRaw);
      if (value) {
        var raw = JSON.stringify(value);
        storageSet(window.localStorage, SESSION_KEY, raw);
        storageSet(window.sessionStorage, SESSION_KEY, raw);
        cookieSet(TOKEN_COOKIE_KEY, value.token);
        if (legacyCookieRaw) cookieRemove(SESSION_KEY);
        return value;
      }
      var token = cookieGet(TOKEN_COOKIE_KEY);
      return token ? { token: token, user: null, tokenOnly: true } : null;
    },
    set: function (value) {
      var session = value || {};
      ...
      storageSet(window.localStorage, SESSION_KEY, raw);
      storageSet(window.sessionStorage, SESSION_KEY, raw);
      if (session.token) cookieSet(TOKEN_COOKIE_KEY, session.token);
  ...
  function applyViewportClasses() {
    ...
    document.documentElement.classList.toggle('ait-mobile', mobile);
    document.documentElement.classList.toggle('ait-compact', compact);
    if (document.body) {
      document.body.classList.toggle('ait-mobile', mobile);
      document.body.classList.toggle('ait-compact', compact);
    }
  }
  function prepareResponsiveTables(root) {
    ...
    for (var i = 0; i < tables.length; i++) {
      ...
      if (!parent || /table-wrap|table-scroll|overflow|scroll/i.test(parent.className || '')) continue;
      var wrap = document.createElement('div');
      wrap.className = 'ait-mobile-table-scroll';
  ...
  window.addEventListener('orientationchange', function(){ setTimeout(function(){ setViewportHeight(); applyViewportClasses(); }, 160); }, { passive: true });
  document.addEventListener('DOMContentLoaded', function(){ applyViewportClasses(); prepareResponsiveTables(document); });
  ...
  function installPageBrand() {
    ...
    var existingImage = bar.querySelector('img[src*="ait-logo"], img.page-logo, img.app-logo, .brand > img');
    if (existingImage) {
      existingImage.classList.add('ait-existing-brand-image');
      return;
    }
  ...
  function installSemanticChartsAndTables() {
    try {
      ...
      function chartText(en){
        var ar=document.documentElement.dir==='rtl';
        if(!ar)return en;
        var dict={
          'No data':'لا توجد بيانات','Tonnage':'الحمولة','Average Fe %':'متوسط نسبة الحديد %','Mine Status':'حالة المنجم',
          'Budget':'الموازنة','Actual':'الفعلي','Spending':'الإنفاق','Monthly Burn':'الإنفاق الشهري',
          'Expenses':'المصروفات','Payment Status':'حالة الدفع','Monthly Expenses':'المصروفات الشهرية',
          'Cost / Ton':'التكلفة / طن','Cost Breakdown':'تفاصيل التكلفة','Total Cost':'إجمالي التكلفة',
          'Inflows':'التدفقات الداخلة','Outflows':'التدفقات الخارجة','Closing Balance':'الرصيد الختامي',
          'Vendor Type':'نوع المورد','Complete Records':'السجلات المكتملة','New Vendors':'الموردون الجدد',
          'Approval Status':'حالة الاعتماد','Requests':'الطلبات','Average Cycle Days':'متوسط أيام الدورة',
          'Closing Stock':'المخزون الختامي','Stock Movement':'حركة المخزون','Stock Balance':'رصيد المخزون',
          'Risk Severity':'شدة المخاطر','Risk Count':'عدد المخاطر','Average Risk Score':'متوسط درجة المخاطر',
          'Mining':'التعدين','Crushing':'التكسير','Transport':'النقل','Port':'الميناء',
          'Received':'الوارد','Delivered':'المنصرف','Returned':'المرتجع','Contact':'بيانات الاتصال','Bank Data':'البيانات البنكية','Email':'البريد الإلكتروني'
        };return dict[en]||en;
      }
      function chart(id,type,map,datasetLabel,color,legend,max) {
        var keys = type === 'line' ? chronological(map) : sortedKeys(map,max);
        if (!keys.length) keys=[chartText('No data')];
        var data = keys[0] === chartText('No data') ? [0] : values(map,keys);
        var ds={label:chartText(datasetLabel),data:data,borderColor:color||palette[0],backgroundColor:type==='line'?(color||palette[0])+'22':(type==='doughnut'?palette.map(function(c){return c+'dd';}):(color||palette[0])+'d9'),borderRadius:type==='bar'?7:0,tension:.35,fill:type==='line',pointRadius:3};
        mkChart(id,type,keys,[ds],{legend:legend !== false});
        return keys[0] === chartText('No data') ? 0 : keys.length;
      }
      ...
      window.renderCharts = function() {
        ...
        if (moduleName === 'mines') {
          ...
          var actual=groupSum(rows,['category','department','cost_center'],['spent','actual','actual_amount','amount']);keys=sortedKeys(a,10);mkChart('c1','bar',keys,[{label:chartText('Budget'),data:values(a,keys),backgroundColor:'#38bdf8cc',borderRadius:7},{label:chartText('Actual'),data:values(actual,keys),backgroundColor:'#fd480edd',borderRadius:7}],{legend:true});
          ...
          b={};b[chartText('Mining')]=0;b[chartText('Crushing')]=0;b[chartText('Transport')]=0;b[chartText('Port')]=0;rows.forEach(function(r){b[chartText('Mining')]+=n(raw(r,['mining_cost','mine_cost']));b[chartText('Crushing')]+=n(raw(r,['crushing_cost','processing_cost']));b[chartText('Transport')]+=n(raw(r,['transport_cost','logistics_cost']));b[chartText('Port')]+=n(raw(r,['port_cost','port_fees']));});
          ...
          var inflow={},outflow={};rows.forEach(function(r){var d=String(raw(r,['month','Month'])||date(r).slice(0,7)).slice(0,7);if(!d)return;inflow[d]=(inflow[d]||0)+n(raw(r,['inflow','cash_in','receipts','collections']));outflow[d]=(outflow[d]||0)+n(raw(r,['outflow','cash_out','payments']));});keys=chronological(Object.assign({},inflow,outflow));mkChart('c1','line',keys,[{label:chartText('Inflows'),data:values(inflow,keys),borderColor:'#22c55e',backgroundColor:'#22c55e22',fill:true,tension:.35},{label:chartText('Outflows'),data:values(outflow,keys),borderColor:'#ef4444',backgroundColor:'#ef444422',fill:true,tension:.35}],{legend:true});
          mkChart('c2','bar',keys,[{label:chartText('Inflows'),data:values(inflow,keys),backgroundColor:'#22c55ecc',borderRadius:7},{label:chartText('Outflows'),data:values(outflow,keys),backgroundColor:'#ef4444cc',borderRadius:7}],{legend:true});
          ...
          b={};b[chartText('Contact')]=0;b[chartText('Bank Data')]=0;b[chartText('Email')]=0;rows.forEach(function(r){if(raw(r,['phone','mobile','contact','contact_person']))b[chartText('Contact')]++;if(raw(r,['bank','bank_name','iban','account_number']))b[chartText('Bank Data')]++;if(raw(r,['email']))b[chartText('Email')]++;});
          ...
          b={};b[chartText('Received')]=0;b[chartText('Delivered')]=0;b[chartText('Returned')]=0;rows.forEach(function(r){b[chartText('Received')]+=n(raw(r,['received','received_qty','qty_in']));b[chartText('Delivered')]+=n(raw(r,['delivered','delivered_qty','issued','qty_out']));b[chartText('Returned')]+=n(raw(r,['returned','returned_qty']));});
  ...
  function miningLoaderMarkup() {
    return '<div class="ait-mining-loader" aria-hidden="true"><svg viewBox="0 0 112 82" focusable="false">' +
      '<path class="ait-loader-ground" d="M13 69.5H101"/>' +
      '<g class="ait-loader-rock"><path d="M64 68 59 60 64 49 75 43 91 47 100 58 96 68Z"/><path class="ait-loader-rock-line" d="m74 46 5 8-5 6 7 8M89 48l-4 7 6 5"/></g>' +
      '<g class="ait-loader-pickaxe"><rect class="ait-loader-handle" x="52" y="13" width="7" height="51" rx="3.5" transform="rotate(-29 55.5 38.5)"/><path class="ait-loader-head" d="M28 20c13-9 31-12 48-5l10 6-4 7-10-4c-13-5-27-2-38 5Z"/></g>' +
      '<path class="ait-loader-chip c1" d="m79 50 5-4 3 6-5 3Z"/><path class="ait-loader-chip c2" d="m85 51 4-5 4 4-3 5Z"/><path class="ait-loader-chip c3" d="m88 55 6-1 1 5-6 1Z"/><path class="ait-loader-chip c4" d="m77 56 4-2 2 4-4 2Z"/>' +
      '<circle class="ait-loader-dust" cx="78" cy="56" r="4"/><circle class="ait-loader-dust" cx="88" cy="58" r="3"/>' +
      '</svg></div>';
  }
  function installMiningLoaders() {
    var selectors = ['.loader .spin','.loader .spinner','.load .spin','.load .spinner','.page-loading .spin','.page-loading .spinner','.loader-screen .spin','.loader-screen .spinner','.box .spin'];
    document.querySelectorAll(selectors.join(',')).forEach(function(spinner){
      if (spinner.closest('.ait-mining-loader')) return;
      var holder = document.createElement('div');
      holder.innerHTML = miningLoaderMarkup();
      var loader = holder.firstElementChild;
      var host = spinner.parentElement;
      if (host) host.classList.add('ait-loader-host');
      spinner.replaceWith(loader);
    });
  }
  function loaderIsVisible(node) {
    if (!node || !node.isConnected) return false;
    if (node.hidden || node.getAttribute('aria-hidden') === 'true') return false;
    var cls = String(node.className || '');
    if (/(^|\s)(hidden|hide|is-hidden)(\s|$)/.test(cls)) return false;
    var style = getComputedStyle(node);
    return style.display !== 'none' && style.visibility !== 'hidden' && parseFloat(style.opacity || '1') > .01;
  }
  function loaderIsLogicallyActive(node) {
    if (!node || !node.isConnected || node.hidden || node.getAttribute('aria-hidden') === 'true') return false;
    var cls = String(node.className || '');
    if (/(^|\s)(hidden|hide|is-hidden|done)(\s|$)/.test(cls)) return false;
    if (/(^|\s)(show|visible|is-visible|active|loading)(\s|$)/.test(cls)) return true;
    var inline = node.style || {};
    if (inline.display === 'none' || inline.visibility === 'hidden' || parseFloat(inline.opacity || '1') <= .01) return false;
    // Some pages render the loader visible by default and only add .hidden/.hide
    // when the API request finishes. Mountain Entry is the inverse: it starts
    // hidden and adds .show while loading. Preserve that original semantic state
    // before the embedded-only CSS visually hides the child loader.
    if (node.__aitLoaderInitiallyHiddenClass && !/(^|\s)(hidden|hide|is-hidden)(\s|$)/.test(cls)) return true;
    if (typeof node.__aitLoaderInitialVisible === 'boolean') return node.__aitLoaderInitialVisible;
    return loaderIsVisible(node);
  }
  function installEmbeddedReadyBridge() {
    if (!EMBEDDED || window.parent === window) return;
    var candidates = Array.prototype.slice.call(document.querySelectorAll('#pageLoad,#pageLoading,.loader-screen,body > .loader,body > .load,body > .page-loading'));
    candidates.forEach(function(node){
      var initialClass = String(node.className || '');
      node.__aitLoaderInitiallyHiddenClass = /(^|\s)(hidden|hide|is-hidden)(\s|$)/.test(initialClass);
      node.__aitLoaderInitialVisible = loaderIsVisible(node);
      node.classList.add('ait-internal-loader','ait-embedded-loader');
    });
    var sent = false, hiddenChecks = 0, started = Date.now(), minimumReadyAt = started + 650;
    function notify(status) {
      if (sent) return;
      sent = true;
      try { window.parent.postMessage({cmd:'aitPageReady',pageKey:CURRENT_PAGE_KEY || '',status:status || 'ready',href:location.href}, '*'); } catch (e) {}
    }
    function check() {
      var visible = candidates.some(loaderIsLogicallyActive);
      hiddenChecks = visible ? 0 : hiddenChecks + 1;
      if (!visible && hiddenChecks >= 3 && Date.now() >= minimumReadyAt) notify('ready');
    }
    if (!candidates.length) {
      setTimeout(function(){notify('dom-ready');}, 700);
      return;
    }
    var observer = new MutationObserver(function(){ hiddenChecks = 0; check(); });
    candidates.forEach(function(node){observer.observe(node,{attributes:true,attributeFilter:['class','style','hidden','aria-hidden']});});
    var timer = setInterval(function(){
      check();
      if (sent || Date.now() - started > 30000) { clearInterval(timer); observer.disconnect(); if (!sent) notify('timeout-fallback'); }
    }, 180);
    window.addEventListener('load',function(){setTimeout(check,220)},{once:true});
    window.AITPageReady = Object.freeze({notify:notify});
  }
  function installMobileFilterToggle() {
    var panels = Array.prototype.slice.call(document.querySelectorAll('.filterbar,.filters,.filter-panel')).filter(function(panel){
      return !panel.closest('.modal,.dialog,.sidebar') && !panel.classList.contains('ait-mobile-filter-panel');
    });
    panels.forEach(function(panel,index){
      var controls = panel.querySelectorAll('input,select,button').length;
      if (controls < 3) return;
      panel.classList.add('ait-mobile-filter-panel');
      var button = document.createElement('button');
      var rtl = document.documentElement.dir === 'rtl' || document.documentElement.lang === 'ar' || safeGet(window.localStorage, 'ait_lang') === 'ar';
      button.type = 'button';
      button.className = 'ait-mobile-filter-toggle';
      button.setAttribute('aria-expanded','false');
      button.setAttribute('aria-controls',panel.id || ('aitMobileFilters' + index));
      if (!panel.id) panel.id = 'aitMobileFilters' + index;
      button.innerHTML = '<span>⚙ <b>' + (rtl ? 'الفلاتر والبحث' : 'Filters & search') + '</b></span><small>' + controls + ' ' + (rtl ? 'خيار' : 'controls') + '</small><i>⌄</i>';
      button.addEventListener('click',function(){
        var open = !panel.classList.contains('ait-mobile-open');
        panel.classList.toggle('ait-mobile-open',open);
  ...
  function showAccessDenied(message) {
    document.documentElement.classList.remove('ait-access-pending');
    document.documentElement.classList.add('ait-access-denied');
    if (!document.body) return;
    var rtl = document.documentElement.dir === 'rtl' || document.documentElement.lang === 'ar';
    var gate = document.createElement('div');
    gate.className = 'ait-access-gate';
    gate.innerHTML = '<div class="ait-access-gate-card"><b>' + (rtl ? 'هذه الصفحة غير متاحة لحسابك' : 'This page is not available for your account') + '</b><p>' + (message || (rtl ? 'سيتم الرجوع إلى أول صفحة مسموح بها.' : 'Returning to the first page you are allowed to open.')) + '</p></div>';
    document.body.appendChild(gate);
    setTimeout(function(){ try { location.replace('Dashboard_Full.html'); } catch (e) {} }, 700);
  }
  function verifyAccessSession(token) {
    return new Promise(function(resolve,reject){
      var cb='ait_page_guard_'+Date.now()+'_'+Math.random().toString(36).slice(2),script=document.createElement('script'),done=false;
      function clean(){done=true;delete window[cb];if(script.parentNode)script.parentNode.removeChild(script);}
      window[cb]=function(data){clean();resolve(data);};
      script.onerror=function(){if(!done){clean();reject(new Error('Connection failed'));}};
      setTimeout(function(){if(!done){clean();reject(new Error('Access check timeout'));}},18000);
      script.src=resolved+'?'+new URLSearchParams({module:'auth',action:'verify',token:token,callback:cb,t:Date.now()}).toString();
      document.body.appendChild(script);
    });
  }
  async function enforceDirectPageAccess() {
    if (!CURRENT_PAGE_KEY) return;
    var session = window.AITSession ? window.AITSession.get() : null;
    var token = String(query.get('token') || (session && session.token) || '');
    if (!token) { try { location.replace('auth.html?reason=expired'); } catch (e) {} return; }
    var user = session && session.user;
    if (!EMBEDDED || !user) {
      try {
        var verified = await verifyAccessSession(token);
        if (!verified || verified.ok === false || !verified.user) throw new Error(verified && verified.message || 'Session expired');
        user = verified.user;
        if (window.AITSession) window.AITSession.set({token:token,user:user,loginAt:session&&session.loginAt||new Date().toISOString()});
      } catch (e) {
        try { if (window.AITSession) window.AITSession.clear(); location.replace('auth.html?reason=expired'); } catch (err) {}
        return;
      }
    }
    if (!userHasPage(user,CURRENT_PAGE_KEY)) { showAccessDenied(); return; }
    document.documentElement.classList.remove('ait-access-pending');
  }
  ...
  function bootEnhancements() {
    ...
    installMiningLoaders();
    installMobileFilterToggle();
    installEmbeddedReadyBridge();
    enforceDirectPageAccess();
    installPageBrand();
    installSemanticKpis();
