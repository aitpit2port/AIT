(function(){
  ...
  window.AITSession = Object.freeze({
    get: function () {
      var raw = storageGet(localStorage, SESSION_KEY) || storageGet(sessionStorage, SESSION_KEY) || cookieGet(SESSION_KEY);
      if (!raw) return null;
      try {
        var value = JSON.parse(raw);
        if (value && value.token) {
          storageSet(localStorage, SESSION_KEY, raw);
          storageSet(sessionStorage, SESSION_KEY, raw);
          return value;
        }
      } catch (e) {}
      return null;
      ...
      window.computeKPIs = function () {
        try {
          ...
          if (moduleName === 'budget') {
            ...
            completed=count(rows,/done|complete|completed|closed/i);var progress=count(rows,/progress|started|active/i),now=new Date(),soon=new Date(now.getTime()+14*86400000),overdue=0,upcoming=0,critical=0;
            rows.forEach(function(r){var dueRaw=rawValue(r,['due_date','planned_finish','end_date','deadline']),due=dueRaw?new Date(dueRaw):null,isDone=/done|complete|completed|closed/i.test(text(r));if(due&&!isNaN(due)){if(due<now&&!isDone)overdue++;if(due>=now&&due<=soon&&!isDone)upcoming++;}if(/critical|high priority|مسار حرج/i.test(String(rawValue(r,['priority','critical_path','is_critical'])||'')))critical++;});
            var health=average(rows,['completion_pct','progress_pct','completion','progress']);
            out=[item(rows.length,'tasks / milestones'),item(completed,'completed'),item(progress,'in progress'),item(overdue||count(rows,/delay|late|overdue|blocked/i),'past due'),item(upcoming,'due within 14 days'),item(critical,'critical activities'),item(pct(health),'average task progress'),item(latest(rows),'schedule data date')];
  ...
  function installTimelineExperience() {
    try {
      ...
      window.AIT_renderTimeline = function(rows) {
        ...
        var normalized = rows.map(function(r,index){var start=parseDate(first(r,['start_date','planned_start','date','Date'])),due=parseDate(first(r,['due_date','planned_finish','end_date','deadline']));if(!start&&due)start=new Date(due.getTime()-5*86400000);if(!due&&start)due=new Date(start.getTime()+5*86400000);return{raw:r,index:index,start:start,due:due,task:first(r,['task','milestone','activity','name'],'Activity '+(index+1)),owner:first(r,['owner','responsible','assigned_to','department'],'Unassigned'),status:first(r,['status','task_status','schedule_status'],'Upcoming'),priority:first(r,['priority','critical_path'],'Normal'),progress:Math.max(0,Math.min(100,parseFloat(first(r,['completion_pct','progress_pct','completion','progress'],0))||0))};});
        var dated=normalized.filter(function(x){return x.start&&x.due;}),min=dated.length?new Date(Math.min.apply(null,dated.map(function(x){return x.start.getTime();}))):new Date(),max=dated.length?new Date(Math.max.apply(null,dated.map(function(x){return x.due.getTime();}))):new Date(Date.now()+30*86400000);if(max<=min)max=new Date(min.getTime()+30*86400000);var span=Math.max(86400000,max-min),mid=new Date(min.getTime()+span/2);
        ...
        normalized.sort(function(a,b){var ac=/critical/i.test(a.priority)?0:1,bc=/critical/i.test(b.priority)?0:1;if(ac!==bc)return ac-bc;return (a.due?a.due.getTime():Infinity)-(b.due?b.due.getTime():Infinity);});
        var shown=normalized.slice(0,9);
        document.getElementById('aitGanttRows').innerHTML=shown.map(function(x){var left=x.start?Math.max(0,Math.min(96,(x.start-min)/span*100)):0,width=x.start&&x.due?Math.max(4,Math.min(100-left,(x.due-x.start)/span*100)):18,cls=statusClass(x.status),critical=/critical/i.test(x.priority);return '<div class="ait-gantt-row"><div class="ait-gantt-task"><b>'+escText(x.task)+'</b><span>'+escText(x.owner)+(critical?' · Critical path':'')+'</span></div><div class="ait-gantt-track"><div class="ait-gantt-bar '+cls+'" style="left:'+left.toFixed(2)+'%;width:'+width.toFixed(2)+'%"><i style="width:'+x.progress+'%"></i><span>'+Math.round(x.progress)+'%</span></div></div><div class="ait-gantt-status"><span class="pill '+(cls==='danger'?'bad':cls==='progress'?'warn':cls==='done'?'good':'')+'">'+escText(x.status)+'</span><small>'+day(x.due)+'</small></div></div>';}).join('') || '<div class="empty">No scheduled activities found.</div>';
        var completed=normalized.filter(function(x){return /done|complete|closed/i.test(x.status);}).length,delayed=normalized.filter(function(x){return /delay|overdue|blocked/i.test(x.status);}).length,critical=normalized.filter(function(x){return /critical/i.test(x.priority);}).length,avg=normalized.length?normalized.reduce(function(s,x){return s+x.progress;},0)/normalized.length:0,health=Math.max(0,Math.min(100,avg-(delayed/Math.max(1,normalized.length))*25));
        ...
        var now=new Date(),actions=normalized.filter(function(x){return !/done|complete|closed/i.test(x.status);}).sort(function(a,b){var ar=/delay|overdue|blocked/i.test(a.status)?0:/critical/i.test(a.priority)?1:2,br=/delay|overdue|blocked/i.test(b.status)?0:/critical/i.test(b.priority)?1:2;if(ar!==br)return ar-br;return (a.due?a.due.getTime():Infinity)-(b.due?b.due.getTime():Infinity);}).slice(0,5);
        document.getElementById('aitTimelineActions').innerHTML=actions.map(function(x){var overdue=x.due&&x.due<now,cls=overdue||/delay|overdue|blocked/i.test(x.status)?'danger':/critical/i.test(x.priority)?'warning':'normal';return '<div class="ait-action '+cls+'"><span></span><div><b>'+escText(x.task)+'</b><small>'+escText(x.owner)+' · '+(overdue?'Overdue ': 'Due ')+day(x.due)+'</small></div><em>'+Math.round(x.progress)+'%</em></div>';}).join('') || '<div class="empty">No open actions.</div>';
      };
    } catch (e) {}
  }
  ...
  function installSemanticChartsAndTables() {
    try {
      ...
      var tableColumns = {
        ...
        timeline:['task','milestone','owner','status','start_date','due_date','completion_pct','delay_days','priority','date'],
