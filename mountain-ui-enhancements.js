...
function openQuickAction(op){
  ...
  input.click();
  window.setTimeout(()=>{
    const tabs=document.querySelectorAll('[data-input-op]');
    const tab=document.querySelector('[data-input-op="'+op+'"]');
    if(tab){tab.click();return;}
    if(tabs.length){
      const close=$('closeModal');if(close)close.click();
      const toast=$('toast');
      if(toast){
        const ar=(document.documentElement.lang||'ar').toLowerCase().startsWith('ar');
        toast.textContent=ar?'هذا الإجراء غير متاح ضمن صلاحيات حسابك.':'This action is not available for your account permissions.';
...
function init(){
  ...
  document.querySelectorAll('[data-quick-op]').forEach(b=>b.addEventListener('click',()=>openQuickAction(b.dataset.quickOp)));
